	...
	if not self.FLAGS.track :
		...
		for b in boxes:
			...
			if self.FLAGS.tracker == "deep_sort":
				...
				#detections = np.append(detections, [left,top,right-left,bot-top])
				#detections = np.append(detections, mess)
		...
		for track in trackers:
			...
			if self.FLAGS.display or self.FLAGS.saveVideo:
				...
				cv2.rectangle(imgcv, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])),
						        (255,255,255), thick//3)
				cv2.line(imgcv, (100,360), (1200,360), (0,0,255), 6)
				cv2.putText(imgcv, ('CAR ='), (10,50), 0, 1, (255,255,255),2)
				#cv2.putText(imgcv, id_num,(int(bbox[0]), int(bbox[1]) - 12),0, 1e-3 * h, (255,255,255),thick//6)

				cv2.putText(imgcv, messi,(120,50),0,1,(0,0,255),thick//3)
	return imgcv
